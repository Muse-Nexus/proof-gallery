...
export function MediaInbox({ busy, onBusyChange, onSaved, onClose, onDirtyStateChange, savedProof = [] }: {
  ...
  useEffect(() => {
    ...
    void refresh();
    return subscribeToLocalProofChanges(kind => {
      // Source consent and auto-saves do not change pending review. Preserve
      // selected rows and drafts instead of treating those notices as refreshes.
      if (kind === "source" || kind === "automatic") return;
  ...
  return <section className="media-inbox" aria-labelledby="media-inbox-title" aria-busy={busy}>
    ...
    <p className="media-guidance">Local and unencrypted, including original file metadata. This one-time picker does not keep folder access or scan your library. Remembered folder access is separate: manage it in Sources. No face recognition, AI analysis, or sync. File modification dates are not treated as event dates.</p>
